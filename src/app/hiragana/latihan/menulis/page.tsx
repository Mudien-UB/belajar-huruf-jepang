...
export default function LatihanMenulisHiragana() {
  ...
  const [showTutorial, setShowTutorial] = useState(() => !hasBeenShown['menulis']);

  // Update hasBeenShown when tutorial is closed for the first time
  const handleCloseTutorial = () => {
    setShowTutorial(false);
    setHasBeenShown('menulis', true);
  };
    ...
    <main className="p-8 flex flex-col items-center relative">
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2">
        <button 
          ...
          className="p-2 bg-navy text-paper rounded-full hover:bg-navy/90"
          aria-label="Tutorial"
        >
